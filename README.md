...
|-------------|-----------------|-----------------|
| `create` | Create a new AI module. | `-n, --name <id>` Name of the module<br>`--template <basic|rlhf|embedding|vision>` Prebuilt module template<br>`--from <path>` Initialize from existing module or config<br>`--config <file>` Custom JSON/YAML configuration for module |
